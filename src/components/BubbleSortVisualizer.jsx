...
function getBubbleSortSteps(inputArr) {
  const steps = [];
  const arr = [...inputArr];
  const n = arr.length;
...
export default function BubbleSortVisualizer() {
  ...
  useEffect(() => {
    const s = getBubbleSortSteps(array);
    setSteps(s);
    setStepIndex(0);
    setPlaying(false);
  }, [array]);
  ...
  return (
    <div className="bg-slate-800/50 border border-blue-700/40 rounded-2xl p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h3 className="text-lg font-bold text-blue-300">🫧 Visualizador — Bubble Sort</h3>
      ...
      <div className="w-full bg-slate-700 rounded-full h-1.5">
        <div className="bg-blue-500 h-1.5 rounded-full transition-all" style={{ width: `${((stepIndex + 1) / steps.length) * 100}%` }} />
      </div>
      ...
      <div className="flex gap-3 justify-center flex-wrap">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg text-sm font-semibold transition-colors"
        >
